...
### 分支层级
```
fix/octree-build-point-format  (开发分支)
  └→ dev  (开发集成，自动推送)
       └→ test  (测试分支，仅手动合并)
            └→ main  (生产发布)
```
...
| 操作 | 说明 |
|------|------|
| `dev` 推送 | 开发分支内容会自动推送到 `dev` |
| `dev → test` 合并 | **必须用户明确说「合并到 test」时才执行**，不会自动合并 |
| `test → main` 合并 | 仅通过 PR review 后合并 |
| 直接推送到 test | **禁止** — 必须从 `dev` 合并过去 |
