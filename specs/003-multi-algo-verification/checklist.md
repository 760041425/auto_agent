...
- [x] 一致性判据比较 H→SLAM XYZ（Z=0）与 NPY XYZ 的三维欧氏距离，不再忽略 Z 分量（AC-003-16）— task #249 验证：0.968m（含 Z）
- [x] 精化步骤复用初始定位匹配器，不再硬编码 LightGlue（AC-003-17）— task #249 验证：1 对失败 → 273 对成功
