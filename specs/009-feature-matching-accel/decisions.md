...
| D-009-10 | macOS + PyTorch 进程禁用 FAISS，自动使用 numpy 检索 | torch/faiss wheel 链接不同 `libomp.dylib`，首次 search 触发 OMP Error #15 并 abort；363 条索引上安全性优先 | `KMP_DUPLICATE_LIB_OK=TRUE` 不安全；统一依赖构建后可重评 | 采用 |
