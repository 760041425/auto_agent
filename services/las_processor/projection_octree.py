...
def _rvec_tvec_to_colmap_line(rvec, tvec, offset_xyz=(0, 0, 0), z_bias: float = 0.0):
    ...
    # 旋转矩阵 → COLMAP 四元数 (qw, qx, qy, qz)
    # 与 _rotation_matrix_to_quaternion 保持一致
...
def render_reprojection_octree(
    octree_dataset: str,
    rvec, tvec,
    camera_matrix,
    img_w: int, img_h: int,
    ...
    替代 _render_point_cloud_splat 的纯 Python 软件渲染，
    使用 C++ 八叉树引擎的高质量渲染（视锥裁剪、深度分层）。

    Args:
        octree_dataset: build_octree 输出的数据集目录
        rvec: PnP 输出的 Rodrigues 旋转向量
        tvec: PnP 输出的平移向量
    ...
    with tempfile.TemporaryDirectory(prefix="octree_reproj_") as tmpdir:
        ...
        ok = render_pose_octree(
            ...
            img_w, img_h, focal_norm,
            color_ppm,
            depth_raw_output=None,
            config=config,
        )

        if not ok or not os.path.exists(color_ppm):
