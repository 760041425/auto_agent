  ...
  try {
    var resp = await fetch(API + '/localize', {
      ...
      body: JSON.stringify({
        ...
        match_methods: ['flann', 'bf', 'flann_lowes', 'bf_cross', 'knn_rank', 'lightglue', 'loftr'],
      }),
    });
    var data = await resp.json();
...
async function pollLocalize(taskId, btn, statusEl, resultsEl) {
  try {
    ...
    results.forEach(function(r) {
      ...
      var matchNames = { 'flann': 'FLANN kd-tree', 'bf': 'BruteForce', 'flann_lowes': 'FLANN严格(0.6)', 'bf_cross': 'BF交叉验证', 'knn_rank': 'KNN Top-50', 'lightglue': 'LightGlue (深度学习)', 'loftr': 'LoFTR (深度学习)' };
