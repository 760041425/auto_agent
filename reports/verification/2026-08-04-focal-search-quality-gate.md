...
| 字段 | 含义 |
|------|------|
| **轮数** | 实际执行的迭代轮数。`0` = 只有初始 PnP（未进入迭代精化）；`1` = 1 轮初始 + 1 轮迭代 |
| **候选** | SALAD 全局检索返回的候选 tile 数（`top_k_retrieval=3`） |
| **焦距搜索尝试/成功** | 每个候选 tile 的 PnP 会触发多阶段焦距搜索（粗 5 次 + 精 2 次 = 7 次/候选），成功次数 = 有解的次数 |

## 轮数详解

- **SALAD+RoMa (原版)**：`轮数=1` 表示完成了 1 轮初始 PnP + 1 轮迭代精化，
  但迭代中 SALAD 相似度未提升，早停逻辑触发，实际只保留初始结果。
- **SALAD v2 / LoFTR / Hybrid**：`轮数=2` 表示完成了 `max_iterations=2` 轮
  （1 轮初始匹配 + 1 轮迭代精化），误差未收敛故跑满。
- **焦距搜索 21/21**：3 候选 × 7 次焦距采样（粗 5 + 精 2）全部成功，
